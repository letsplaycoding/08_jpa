...
/* 설명.
 *  Service 계층: 비즈니스 로직, 트랜잭션처리(@Transactional), DTO <-> Entity(modelmapper library 활용)
* */
@Service
public class MenuService {
    ...
    public MenuDTO findMenuByCode(int menuCode) {
        ...
        Menu menu = menuRepository.findById(menuCode).orElseThrow(IllegalArgumentException::new);// 반환형이 Optional일 때 .orElseThrow() 를 사용하자
    ...
    public Page<MenuDTO> findMenuList(Pageable pageable) {
        ...
        /* 설명.
         *  1.넘어온 pageabe에 담긴 페이지 번호를 인덱스 개념으로 바꿔서 인지 시킴.
         *  2. 한 페이지에 뿌려질 데이터 크기
         *  3. 정렬 기준
        * */
